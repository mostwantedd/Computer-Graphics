...
PolyMesh2::PolyMesh2(char *file, Transform *transform)
{
  this->do_construct(file, transform);
}

void PolyMesh2::do_construct(char *file, Transform *transform)
{
  int count;
...
// Moller-Trumbore
bool PolyMesh2::rayTriangleIntersect(const Ray& ray, const Vector &v0, const Vector &v1, const Vector &v2, float &t)
{
  Vector p;
  Vector d;
  Vector e1,e2,h,s,q;
...
void PolyMesh2::compute_vertex_normals(void)
{
  int i,j;
...
void PolyMesh2::compute_face_normal(int which_triangle, Vector &normal)
{
  Vector v0v1, v0v2;
...
void PolyMesh2::triangle_intersection(Ray ray, Hit &hit, int which_triangle)
{
  hit.flag = false;
...
void PolyMesh2::intersection(Ray ray, Hit &hit)
{
  Hit current_hit;
  int i;
